...
**Errors and spinners only when there is nothing to show.** A failed background refetch leaves the existing list on screen rather than replacing it with an error, so a refresh that fails offline never costs the user the reports they were reading. The missing half is reporting the failure at all. Pull to refresh is the only refetch trigger in this build — focus and reconnect refetching both need `AppState` and NetInfo wired into TanStack's managers, which is out of scope here — so every refetch is an explicit user request, and a failed one produces a spinner that simply stops. Visually identical to a refresh that succeeded and returned the same data.

What I would build is two surfaces rather than one. A dismissible banner carrying `ReportsError.userMessage` answers *why*, so a 503 says the server failed instead of telling someone to check a connection that is fine — the taxonomy above already has the copy, it just has nowhere to render once the list is non-empty. An "Updated N ago" stamp from the query's `dataUpdatedAt` answers *how stale*, and is the half that keeps earning when nothing has failed. Not a toast: the condition it reports stays true long after the toast fades, so anyone who looked away is uninformed again — which is the bug being fixed. This is the next thing I would add here.
