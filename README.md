...
2. **Observability — an error boundary, crash reporting, and structured logs.** Today an unhandled render throw takes the whole tree down: React unmounts on error, so the user gets a blank screen and I get nothing. A boundary above the navigator, with a per-screen fallback and a retry, turns that into a recoverable state instead of a dead app. Sentry's React Native SDK then captures both the JavaScript throw and native crashes out of the Kotlin module, symbolicated as long as source maps are uploaded against a matching release and dist.
